...
unit strings with an active-learning loop that calibrates each unconverted unit against
already-converted anchors and admits it only when it passes the same Homoset noise gate
(\num{22} units, \num{+268} molecules, and \emph{improved} FreeSolv agreement); and (ii) use the resulting redundant,
...
acceptance criterion. Let the \emph{anchors} be the molecules already assigned a $\dGhyd$ by the
physics routes. For each unconverted (unit, process) group we fit the best monotone map
...
\emph{admit} the calibration iff it both correlates ($|R|\ge0.85$) and passes the noise gate
($\mathrm{RMS}/L\le\eta$, $\eta=0.35$). An admitted unit converts all of its rows, whose
per-molecule medians become new anchors, so the next round can calibrate units that only overlap
the newly-reached molecules; the loop stops when a round admits nothing.

The loop admits \num{22} units in two rounds (\num{12} at high confidence: Henry/free-energy
processes with $|R|\ge0.95$ and $\mathrm{RMS}/L\le0.15$), adding \num{5244} observations and
\num{268} previously-unreachable molecules (\num{2473}$\to$\num{2741}). It is self-validating: it
independently recovers the air/water sign of the dimensionless Henry constant --- $\log(M/M)$ is
fit with slope $-0.99$ under the gas/water process and $+1.00$ under the water/gas process --- and
it cracks opaque strings a human would struggle with, e.g.\ \texttt{MPv/(RTCw)} ($\ln$, $R=0.99$)
and \texttt{logK=y/x at 1 atm} ($R=0.99$). Crucially the expansion \emph{improves} accuracy: the
extra corroborating observations sharpen the per-molecule medians, taking the FreeSolv agreement
from \SI{0.399}{\kcal} ($R=0.948$) to \SI{0.328}{\kcal} ($R=0.961$) even as coverage grows.
FreeSolv is never used as an anchor, so this is a held-out check.
...
scripts (\texttt{harmonize\_guthrie.py}, \texttt{calibrate\_units\_loop.py},
\texttt{compare\_methods.py}, \texttt{validate\_vs\_freesolv.py}) are in
\texttt{homoset\_vs\_mixle/} on the \texttt{homoset} branch of the repository.
