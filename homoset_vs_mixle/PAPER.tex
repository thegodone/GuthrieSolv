...
``homogeneous set'' by aligning sources with a median offset and admitting them only when a
proportional-similarity (PS) index clears a $\chi^2$ critical value. We apply that classical
odour-threshold machinery, unchanged, to hydration free energies. The method treats each
measurement type as a data \emph{source} and the direct
