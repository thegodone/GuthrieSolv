...
(\num{26} units, \num{+290} molecules, and \emph{improved} FreeSolv agreement); and (ii) use the resulting redundant,
...
iteration continues until a pass at the loosest gate admits nothing. It converges in six outer
steps, admitting \num{26} units and \num{5446} observations and reaching \num{290}
previously-unreachable molecules (\num{2473}$\to$\num{2763}). Accuracy is dominated by the strict
first step: the held-out FreeSolv agreement drops from \SI{0.399}{\kcal} ($R=0.948$) to
\SI{0.321}{\kcal} ($R=0.962$) and then holds flat ($\approx$\SI{0.32}{\kcal}) as later steps add
coverage at neutral cost --- a clean coverage/accuracy Pareto with a self-terminating stop
(FreeSolv is never an anchor). The loop is self-validating: it independently recovers the
...
Homoset, $L$ adaptive, $\eta{=}0.50$    & 0.392 & 1.159 & 0.202 & 0.574 & 1.402 & $+0.22$ \\
Homoset, $L$ adaptive, $\eta{=}0.25$    & 0.350 & 1.068 & 0.159 & 0.491 & 1.248 & $+0.19$ \\
\textbf{Homoset, $L{=}0.6$ (any $\eta$)} & \bfseries 0.283 & 0.979 & \bfseries 0.083 & 0.357 & 1.071 & $+0.15$ \\
\textbf{mixle, gaussian}                & 0.308 & \bfseries 0.866 & 0.141 & 0.359 & \bfseries 0.855 & \bfseries $+0.07$ \\
mixle, robust-$t$                       & 0.276 & 0.949 & 0.116 & \bfseries 0.354 & 1.060 & $+0.09$ \\
\bottomrule
\end{tabular}
\caption{Accuracy against FreeSolv on the overlap ($n=556$; \si{\kcal}); conflict column over
the 278 molecules whose replicates fail the homogeneity test. Bold marks the best value in each
column. Homoset is shown across its two parameters, the dimension $L$ and the noise level
$\eta$; at $L{=}0.6$ both noise levels reject every converted source and so score identically.}
\label{tab:main}
\end{table}
...
data-adaptive $L$ (here \SIrange{5.9}{10.3}{\kcal}, inflated by a few wildly disagreeing
molecules) the sources have noise $0.14$--$0.34$; a noise level $\eta=0.25$ admits the two
cleanest converted sources (KWG, PAIR) but rejects the Henry gas/water source, while
$\eta=0.50$ admits all three, and the consensus barely improves on a plain median. With the
fixed physical $L=\SI{0.6}{\kcal}$ the converted-source noise is $1.4$--$5.0$, far above either
noise level, so all converted sources are rejected and only the direct free-energy records
survive: the gate becomes a hard source-quality filter, and this strict setting attains the best
median-based accuracy. Notably, the source the gate is most reluctant to admit --- Henry
gas/water, the only one rejected at $\eta=0.25$ --- is exactly the source the hierarchical model
assigns the largest bias (\SI{+1.18}{\kcal}, below), an independent corroboration of which
source is least trustworthy.
...
\midrule
KWG (Henry water/gas)          & 0.14 & 1.36 & $\eta\ge0.25$ ($L$ adaptive) \\
PAIR (vapour-P $\times$ sol.)  & 0.18 & 3.01 & $\eta\ge0.25$ ($L$ adaptive) \\
KGW (Henry gas/water)          & 0.34 & 5.03 & $\eta\ge0.50$ ($L$ adaptive) \\
\bottomrule
\end{tabular}
